...
Nota PageSpeed 2026-07-09: en la rama `codex/pagespeed-100`, el tema avanza a 1.6.2 con mapas Google bajo demanda, logos/retrato WebP/AVIF más pequeños, preload/srcset refinado para LCP y caché pública anónima de página en Nginx. La medición final en producción sigue pendiente antes de fusionar.
...
- Rama 1.6.2 local/VPS: `cmd /c npm run build` correcto, `node --check` correcto para `navigation.js` y `animations.js`, mapas Google sin `src` inicial en templates, logo/retrato WebP/AVIF reducidos, sin referencias frontend activas a Google Fonts/CDN/Alpine/GSAP, lint PHP correcto dentro del contenedor y Nginx con caché `MISS`/`HIT` para home pública y `BYPASS` en admin/API. Lighthouse local móvil válido alcanzó 99/100/100/100 antes del AVIF final para LCP.
